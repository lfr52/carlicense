...
def find_car_brod():
    ...
    # 先读取图片
    image = cv2.imread("./cut/test_img/car2.jpg")
    resize_h = 1000
    ...
    for (x, y, w, h) in watches:
        ...
        cv2.imwrite("./cut/car2.jpg", cut_gray)
        im = Image.open("./cut/car2.jpg")
        size = 720, 180
        mmm = im.resize(size, Image.ANTIALIAS)
        mmm.save("./cut/car2.jpg", "JPEG", quality=90)
        break
...
def cut_car_num_for_chart():
    # 读取图像，并把图像转换为灰度图像并显示
    img = cv2.imread("./cut/car2.jpg")  # 读取图片
    ...
    cv2.imwrite('./cut/car2_binary.jpg', th3)
    # cv2.waitKey(0)
        ...
        if (white[n] if arg else black[n]) > (0.05 * white_max + 1 if arg else 0.05 * black_max):
            ...
            if end - start > 5:  # 移除车牌白条
                ...
                if temp == 1 and end - start < 30:
                    ...
                    shutil.copy(
                        os.path.join("./cut/1", "111.bmp"),  # 111.bmp 是一张 1 的样本图片
                        os.path.join("./cut\\img_cut\\", str(temp + 16) + '.bmp'))
                    temp = temp + 1
                    pass
                else:
                    cj = th3[1:height, start:end]
                    cv2.imwrite("./cut\img_cut_initial\\" + str(temp + 16) + ".jpg", cj)

                    im = Image.open("./cut\img_cut_initial\\" + str(temp + 16) + ".jpg")
                    size = 32, 40
                    mmm = im.resize(size, Image.ANTIALIAS)
                    mmm.save("./cut\img_cut\\" + str(temp + 16) + ".bmp", quality=95)
                    temp = temp + 1
