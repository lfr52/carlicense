...
if __name__ =='__main__' and sys.argv[1]=='predict': 
    ...
    with tf.Session() as sess:
        ...
        l=[]
        num = 0 
        for rt, dirs, files in os.walk( "data/test_images/"):
            for filename in files:
                num += 1

        for n in range(1, num + 1):
            print(n)
            path = "data/test_images/%s.bmp" % (n)
            ...
            output = "概率：  [%s %0.2f%%]    [%s %0.2f%%]    [%s %0.2f%%]" % (PROVINCES[max1_index],max1*100, PROVINCES[max2_index],max2*100, PROVINCES[max3_index],max3*100)
            print (output)
        ...
        file = open(OUTPUT_DIR + 'result.txt', 'w')
        for n in range(1, num + 1):
            file.write("%s %s\n" %(n , str(l[n-1])))
        file.close()    
