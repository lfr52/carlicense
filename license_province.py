...
from pic_prepare import reverse
from pic_prepare import pic_count
from pic_prepare import pic_intput
...
for n in range(0, NUM_CLASSES):
    ...
    # print(a)
    # print(a[-2:-1])
    PROVINCES.append(a[-2:-1])
file.close()




# 定义输出节点，对应于图片像素值矩阵集合和图片标签
...
def province_train(SIZE=1280, WIDTH=32, HEIGHT=40, NUM_CLASSES=37, iterations=200):
    time_begin = time.time()
    # 获取图片总数
    input_count = pic_count('data/training_set/', NUM_CLASSES)
    ...
    for i in range(0, NUM_CLASSES):
        dir = 'data/training_set/%s/' % i  # 这里可以改成你自己的图片目录，i为分类标签
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                input_images[index] = pic_intput(dir + filename, input_images[index], SIZE)
                input_labels[index][i] = 1
                index += 1

                # 获取图片总数
    val_count = pic_count('data/vaildation_set/', NUM_CLASSES)
    ...
    for i in range(0, NUM_CLASSES):
        ...
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                val_images[index] = pic_intput(dir + filename, val_images[index], SIZE)
    ...
    with tf.Session() as sess:
        ...
        for it in range(iterations):
            ...
            if it % 5 == 0:
                ...
                if iterate_accuracy >= 0.9999 and it >= 100:
                    break
        ...
        # 保存训练结果
        if not os.path.exists(SAVER + "province/"):
            print('不存在训练数据保存目录，现在创建保存目录')
            os.makedirs(SAVER + "province/")
        saver_path = saver.save(sess, "%smodel.ckpt" % (SAVER + "province/"))


def province_test(file_path, NUM_CLASSES=37):
    saver = tf.train.import_meta_graph("%smodel.ckpt.meta" % (SAVER + "province/"))  ########################
    with tf.Session() as sess:
        model_file = tf.train.latest_checkpoint(SAVER + "province/")
        ...
        for n in range(0, num):
            ...
            output = "%s 概率：  [%s %0.2f%%]    [%s %0.2f%%]    [%s %0.2f%%]" % (
            ll[n], PROVINCES[max1_index], max1 * 100, PROVINCES[max2_index], max2 * 100, PROVINCES[max3_index], max3 * 100)
            print(output)
        ...
        file = open('test_result/result_province.txt', 'w')
