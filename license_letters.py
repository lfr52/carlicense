...
if __name__ =='__main__' and sys.argv[1]=='predict': 
    ...
    with tf.Session() as sess:
        ...
        for n in range(1, num + 1):
            print(n)
            path = "F:\DeepLearning\lfr\\t/%s.bmp" %(n )
            ...
            output = "概率：  [%s %0.2f%%]    [%s %0.2f%%]    [%s %0.2f%%]" % (LETTERS_DIGITS[max1_index],max1*100, LETTERS_DIGITS[max2_index],max2*100, LETTERS_DIGITS[max3_index],max3*100)
            print (output)
        ...
        # file = open(OUTPUT_DIR + 'result_letters.txt', 'w')
        # for n in range(1, num + 1):
        #     file.write("%s %s\n" %(n , str(l[n-1])))
        # file.close()    
