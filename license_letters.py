...
def letter_train(SIZE=1280, WIDTH=32, HEIGHT=40, NUM_CLASSES=34, iterations=200):
    time_begin = time.time()
    # 获取图片总数
    input_count = pic_count('data_letters/training_set/', NUM_CLASSES)
    ...
    for i in range(0, NUM_CLASSES):
        dir = 'data_letters/training_set/%s/' % i  # 这里可以改成你自己的图片目录，i为分类标签
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                ...
                # 获取图片总数
    val_count = pic_count('data_letters/vaildation_set/', NUM_CLASSES)
    ...
    index = 0
    for i in range(0, NUM_CLASSES):
        dir = 'data_letters/vaildation_set/%s/' % i  # 这里可以改成你自己的图片目录，i为分类标签
        for rt, dirs, files in os.walk(dir):
            for filename in files:
...
def letter_test(file_path, NUM_CLASSES=34):
    ...
    with tf.Session() as sess:
        ...
        l = []
        ll = []
        fname_list = os.listdir(file_path) # "F:\DeepLearning\lfr/t2/"
        for i in fname_list:
            if i[-6:] != '_1.jpg':
                ll.append(i)
        num = len(ll)
        ...
        for n in range(0, num):
            ...
            for j in range(NUM_CLASSES):
                ...
                if result[0][j] > max3:
                    ...
                    max3_index = j
                    continue
            nLettersIndex = max1_index
            output = "%s  概率：  [%s %0.2f%%]    [%s %0.2f%%]    [%s %0.2f%%]" % (
            ll[n], LETTERS_DIGITS[max1_index], max1 * 100, LETTERS_DIGITS[max2_index], max2 * 100, LETTERS_DIGITS[max3_index],
            max3 * 100)
            print(output)
            print("字符是: %s" % LETTERS_DIGITS[nLettersIndex])
            l.append(output)

        if not os.path.exists('test_result'):
            os.makedirs('test_result')
        file = open('test_result/result_letters.txt', 'w')
